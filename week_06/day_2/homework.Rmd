...
phones %>% 
  select(brand) %>% 
  group_by(brand) %>% 
  summarise(total = n(brand))
```
```{r}
phones %>% 
  ggplot() +
  aes(x = brand, stat = "count") +
  geom_bar()
```
...
samsung_review_stats

samsung_reviews <- phones %>% 
  filter(brand == "Samsung") %>% 
  ggplot() +
  aes(x = rating) +
  ...
  stat_function(
    fun = dnorm,
    args = list(
      mean = samsung_review_stats$mean,
      sd = samsung_review_stats$sd
    ),
    col = "red"
  )
samsung_reviews
```
The normal distribution looks like a reasonable fit of the distribution, since most observations are located around a central value.
<br>
...
Expected proportion of ratings within 1sd of the mean is 67%
```{r}
pnorm(q = 1, mean = 0, sd = 1) - pnorm(q = -1, mean = 0, sd = 1)
```
The mean from the data is 3.5733	and the sd is 0.6847859
```{r}
phones %>% 
  filter(brand == "Samsung",
         between(rating, left = 3.5733 - (1 * 0.6847859), right = 3.5733 + (1 * 0.6847859))) %>% 
  summarise(count = n())
```
```{r}
315/397
```
80% of values are within 1 sd of the data, which is higher than the expectation of 67%, so perhaps the data is not normally distributed.


Expected proportion of ratings within 2sd of the mean is 95%
```{r}
...
phones %>% 
  filter(brand == "Samsung",
       between(rating, left = 3.5733 - (2 * 0.6847859), right = 3.5733 + (2 * 0.6847859))) %>% 
  summarise(count = n())
```
```{r}
365/397
```
92% of values are within 2 sd of the data, which is lower than the expectation of 95%, so perhaps the data is not normally distributed.

Expected proportion of ratings within 3sd of the mean is 99%
```{r}
...
phones %>% 
  filter(brand == "Samsung",
       between(rating, left = 3.5733 - (3 * 0.6847859), right = 3.5733 + (3 * 0.6847859))) %>% 
  summarise(count = n())
```
...
389/397
```
98% of values are within 3 sd of the data, which is similar to the expectation of 99%, so perhaps the data is normally distributed on this metric.

In summary, the data may not be normally distributed, since the dispersion seems more narrow than a normal distribution would indicate.
