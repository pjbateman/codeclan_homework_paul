...
<br>
```{r}
# the r value of 80 indicates strong positive correlation between the two variables
pm_data %>% 
...
<br>
```{r}
# the actual amount of time taken for a project is 1.22 times the estimate time
# the r2 of 65% indicates a good fit, but there may be another variable which explains the actual time take for a project
```
...
library(ggfortify)
autoplot(pm_model)
# the plots looks broadly fine
# yes, the low p-value of estimated_length of 1.91e-12 is statistically significant
```
