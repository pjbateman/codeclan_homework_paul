...
1.1 Are the Thought Police ever mentioned?
```{r}
library(tidyverse)

# thinking of whether there could be a lower case occurrence of 'thought police'
pattern <- c("[tT]hought [pP]olice")
...
1.2 Count how often Big Brother is mentioned
```{r}
# want to capture various instances of capitalisation format of the phrase
pattern <- c("[bB][iI][gG] [bB][rR][oO][tT][hH][eE][rR]")
...
1.3 The chapter is written from the point of view of the protagonist, Winston. Does he ever mention the other main character (Julia) by name?
```{r}
pattern <- c("[jJ]ulia")
...
1.4 Winston writes the date in a specific format: month nth, year eg. August 5th, 2019. Write a regular expression to match dates in that format and find the date written in his diary.
```{r}
# finding month that begins with a capital, then has some lowercase text
# finding the day with a space after the month followed by 2 lowercase character
# find the 4-digit number for year, which follows a comma with a space
...
1.5 Winston wants some anonymity, redact his name somehow.
```{r}
pattern_W <- ("[wW]inston")

whole_text %>% 
  str_replace_all(pattern_W, "REDACTED") %>% 
  # check that Winston is not there
  str_detect(pattern_W)
```




1.6 Stocks of Victory Gin have run out and the people of Oceania will have to drink vodka instead. Replace “gin” with “vodka”, matching case wherever possible. There’s still loads of Victory Coffee, though, so don’t replace that.
```{r}
gin_pattern <- (" gin ")
Gin_pattern <- (" Gin ")
GIN_pattern <- (" GIN ")
...
1.7 Big Brother is rebranding and has decided that the various ministries will now be known as departments. The old departments are now called sections. The Newspeak titles will use “Dep” instead of “Mini”.
```{r}
